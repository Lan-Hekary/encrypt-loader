...
function encrypt(content, options) {
	...
	if (options.minify) {
		try {
			let { code } = UglifyJS.minify(
				options.minifyPrefix + content,
				options.uglifyOptions
			);
			content = code.substr(options.minifyPrefix.length);
		} catch (err) {
			process.stdout.write(
				`encrypt-loader minify fail: ${this.resourcePath}\n`
			);
		}
	}
...
module.exports = function (source) {
	...
	if (options.mode == "file") {
		...
		let tagReg = new RegExp(
			`(?:/\\*\\s*<${options.tag}>\\s*\\*/|//\\s*<${options.tag}>)([\\s\\S]*?)(?:/\\*\\s*</${options.tag}>\\s*\\*/|//\\s*</${options.tag}>)`,
			"g"
		);
		source = source.replace(tagReg, (m, cont) =>
			encrypt.call(self, cont, options)
		);
	}
